...
construction. They fix the combination estimand, the
...
Two families of methods do this at different stages. Family~(a)
combines \emph{before} scoring: a multivariate model reweights
variant-level summaries, as in MTAG or wMT-SBLUP
\cite{turley2018,maier2018}. Family~(b) builds one score per
source and combines the scores. Early demonstrations used a
handful of scores with weights learned in a target population
\cite{marquez2017,krapohl2018}. MetaGRS deployed a same-trait
stack as a single weight file \cite{inouye2018}. Albi\~nana et
al.\ stacked 937 PGS Catalog scores by elastic-net regression
\cite{albinana2023,lambert2021}. PRSmix/PRSmix+ later split
same-trait combination from cross-trait combination
\cite{truong2024}. Family~(c) derives the combination weights
from metadata rather than from a target phenotype; wMT-SBLUP
already does this at the predictor level
\cite{maier2018,smith1936,hazel1943}.
...
mean, or a causal decomposition of the panel. A high ranking
metric is not any of those things.
...
\begin{enumerate}
  ...
  means are particularly unsafe across ancestries.
...
hypotheses until an untouched outcome is scored.
