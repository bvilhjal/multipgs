...
- **Timothy Shin Heng Mak, Robert Milan Porsch, Shing Wan Choi, Xueya Zhou, Pak Chung Sham** (2017). Polygenic scores via penalized regression on summary statistics. *Genetic Epidemiology 41(6):469–480*. [doi:10.1002/gepi.22050](https://doi.org/10.1002/gepi.22050)
  The summary-statistic form of the same penalized-regression objective, and the canonical answer to 'how do you tune lambda with no validation phenotype' — the question CMSA answers differently. Its pseudovalidation criterion is what `multi_pgs_sumstats` maximises over the path, and it carries the same in-sample optimism.
- **Zhipeng Zhao, Timothy Gruenloh, Meiyi Yan, Yuchang Wu, Zhongxuan Sun et al.** (2024). Optimizing and benchmarking polygenic risk scores with GWAS summary statistics. *Genome Biology 25:260*. [doi:10.1186/s13059-024-03400-w](https://doi.org/10.1186/s13059-024-03400-w)
  **The direct precedent for the summary-statistic backend.** PUMAS-ensemble / PUMA-CUBS recursively partitions one GWAS into training / tuning / ensemble-learning / testing subsets and fits regression weights combining several PRS into one ensemble score with no external data — which is what `multi_pgs_sumstats(tune="pumas")` does, reformulated in score space. Cite this before the 2021 paper for the ensemble idea.
- **Zhipeng Zhao, Yuchang Wu, Jiacheng Miao, Yixuan Song, Xiaoyuan Zhong et al.** (2021). PUMAS: fine-tuning polygenic risk scores with GWAS summary statistics. *Genome Biology 22:257*. [doi:10.1186/s13059-021-02479-9](https://doi.org/10.1186/s13059-021-02479-9)
  The subsampling scheme itself, for a single PRS. `subsample_score_moments` is its score-space image: the K-dimensional draw is the exact pushforward `W^T(var_y·D + z z^T)W` of the variant-space law, so no m x m matrix square root is needed. Note this is PUMAS-*style* — the exact Gaussian moment is used rather than the paper's SE-based plug-in, because in score space the rank-one term is each score's own R² and is not negligible.
- **Florian Privé, Julyan Arbel, Hugues Aschard, Bjarni J. Vilhjálmsson** (2022). Identifying and correcting for misspecifications in GWAS summary statistics and polygenic scores. *Human Genetics and Genomics Advances 3(4):100136*. [doi:10.1016/j.xhgg.2022.100136](https://doi.org/10.1016/j.xhgg.2022.100136)
  lassosum2, and the reason `multi_pgs_sumstats` does not inherit lassosum's local-FDR pseudovalidation: the authors report it was not robust enough for an automatic mode. The separately-tuned LD-stabilisation parameter is kept; the tuning criterion is not.
