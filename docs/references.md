...
- **Xu L, Dong Y, Zeng X, Bian Z, Zhou G, Guan L, Zhao H** (2026). MIXPRS enables multi-population and multi-method polygenic risk scores using summary statistics. *Nature Genetics 58, 1583-1594*. [doi:10.1038/s41588-026-02637-4](https://doi.org/10.1038/s41588-026-02637-4)
  Same-trait ensemble of multi-population *methods* (JointPRS-auto and SDPRX). Combination weights are non-negative least squares from a data-fission pseudo-split of one target GWAS; SNP pruning plus an identity residual is used to limit LD-mismatch inflation of those residuals. The panel is method-by-ancestry versions of one trait, not Catalog scores of many traits. The paper lists PRS for related traits as future work — that is the Albiñana / `multi_pgs_fit` problem. This package does not implement MIXPRS.
...
- **Timothy Shin Heng Mak, Robert Milan Porsch, Shing Wan Choi, Xueya Zhou, Pak Chung Sham** (2017). Polygenic scores via penalized regression on summary statistics. *Genetic Epidemiology 41(6):469–480*. [doi:10.1002/gepi.22050](https://doi.org/10.1002/gepi.22050)
  The SNP-level summary-statistic penalized-regression precedent. `multi_pgs_sumstats` uses the same quadratic form over K component-score coefficients rather than m SNP effects; its lasso therefore selects whole scores and cannot estimate effects outside their span.
- **Zhipeng Zhao, Timothy Gruenloh, Meiyi Yan, Yuchang Wu, Zhongxuan Sun et al.** (2024). Optimizing and benchmarking polygenic risk scores with GWAS summary statistics. *Genome Biology 25:260*. [doi:10.1186/s13059-024-03400-w](https://doi.org/10.1186/s13059-024-03400-w)
  The direct ensemble precedent. PUMAS-ensemble / PUMA-CUBS recursively partitions one GWAS into training, tuning, ensemble-learning, and testing subsets. `multi_pgs_sumstats(tune="pumas")` implements a narrower two-way score-space pseudotuning step; it does not reproduce that four-stage assessment design.
- **Zhipeng Zhao, Yuchang Wu, Jiacheng Miao, Yixuan Song, Xiaoyuan Zhong et al.** (2021). PUMAS: fine-tuning polygenic risk scores with GWAS summary statistics. *Genome Biology 22:257*. [doi:10.1186/s13059-021-02479-9](https://doi.org/10.1186/s13059-021-02479-9)
  The single-PRS pseudotuning scheme. `subsample_score_moments` uses an analogous joint-Gaussian/CLT plug-in covariance after projection into score space. That covariance is an approximation, not a distribution-free identity, and it is valid for tuning only when the component weights were constructed independently of the target GWAS being split.
- **Florian Privé, Julyan Arbel, Hugues Aschard, Bjarni J. Vilhjálmsson** (2022). Identifying and correcting for misspecifications in GWAS summary statistics and polygenic scores. *Human Genetics and Genomics Advances 3(4):100136*. [doi:10.1016/j.xhgg.2022.100136](https://doi.org/10.1016/j.xhgg.2022.100136)
  lassosum2, and the reason `multi_pgs_sumstats` does not inherit lassosum's local-FDR pseudovalidation: the authors report it was not robust enough for an automatic mode. It also motivates reporting external-LD compatibility diagnostics. The separately tuned LD-stabilisation parameter is kept; the tuning criterion is not.
- **Jack W. Pattee, Wei Pan** (2020). Penalized regression and model selection methods for polygenic scores on summary statistics. *PLOS Computational Biology 16(10):e1008271*. [doi:10.1371/journal.pcbi.1008271](https://doi.org/10.1371/journal.pcbi.1008271)
  The quasi-correlation criterion `(w'z)^2 / (w'Dw)`, which is the accuracy `multipgs.sumstat` evaluates for the combined score `W beta`, and a direct treatment of model selection for penalized regression on summary statistics — the open problem in the summary-statistic fit.
...
- **Alicia R. Martin, Masahiro Kanai, Yoichiro Kamatani, Yukinori Okada, Benjamin M. Neale, Mark J. Daly** (2019). Clinical use of current polygenic risk scores may exacerbate health disparities. *Nature Genetics 51(4):584-591*. [doi:10.1038/s41588-019-0379-x](https://doi.org/10.1038/s41588-019-0379-x)
  The canonical statement of the portability limit arising from Eurocentric discovery GWAS. It predates the PGS Catalog and therefore supports the general transferability warning, not a numerical claim about the Catalog's current ancestry composition.
